...
import { DownloadCloud, FolderOpen, Heart, Play, Trash2 } from 'lucide-vue-next'
...
const { localTracks } = storeToRefs(libraryStore)
const { currentTrack } = storeToRefs(playerStore)

const visibleTracks = computed<LocalMusicTrack[]>(() =>
  localTracks.value.map((track) => ({
    ...track,
    isFavorite: libraryStore.isFavorite(track.id),
  })),
)
const hasTracks = computed(() => visibleTracks.value.length > 0)
const favoriteCount = computed(() => visibleTracks.value.filter((track) => track.isFavorite).length)
      ...
      <section class="local-board">
        ...
        <div class="local-board__body">
          <div v-if="!hasTracks" class="local-state">
            ...
            <span>还没有本地歌曲，先在播放器左下角点一下下载。</span>
          </div>
